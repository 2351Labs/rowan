...
Package name: `@rowan-ui/core`. Version `0.11.0`. User-visible changes are in
[`CHANGELOG.md`](CHANGELOG.md).

**`0.11` surface.** Public API is attributes, properties, slots, events, tokens,
...
- Actions and status: `rowan-button`, `rowan-icon-button`, `rowan-link`, `rowan-badge`, `rowan-chip`, `rowan-avatar`, `rowan-alert`, `rowan-status-indicator`, `rowan-spinner`, `rowan-progress`, `rowan-skeleton`, `rowan-divider`, `rowan-empty-state`, `rowan-data-state`, `rowan-toast`, `rowan-toaster`, `rowan-file-item`
...
are **out of `0.11`** until they are marked Stable. The listed dashboard hosts
...
not fetch data or emit events; retry is the author's button. Table and KPI keep
their own `loading`.

```html
<rowan-data-state state="error">
  <p>Fill rate is 82%.</p>
...
`rowan-app-layout` composes a responsive header, primary navigation, and default main-content slot. It keeps navigation state reflected through `navigation-open`; only user use of the compact toggle, backdrop, or Escape emits `rowan-change`. Compose `rowan-side-nav-item` children in `rowan-side-nav` for application destinations (Arrow, Home, End, Enter, Space). Wrap items in `rowan-side-nav-section` for labeled groups; the parent nav still has one `value`. Set `collapsible` for a disclosure control; `collapsed` defaults to open. User expand/collapse emits `rowan-toggle` on the section (not `rowan-change` on the nav). Arrow Left collapses the containing group; setting `value` to a child opens ancestor groups. An empty `value` means nothing is selected. Items accept `tone` and optional `count` for exception cues; a slotted `suffix` replaces the built-in status.
